 ...
 */
@SuppressWarnings({"PointlessArithmeticExpression", "WeakerAccess", "unused"})
public class JPEGDecoder {
     ...
     * @see #decodeDCTCoeffs(ShortBuffer[], int)
     */
    ...
    private void processSOF() throws IOException {
        ...
        for (int i = 0; i < numComps; i++) {
            ...
            if (c.blocksPerMCUHorz == 0 || c.blocksPerMCUHorz > 4) throw new IOException("bad H");
            if (c.blocksPerMCUVert == 0 || c.blocksPerMCUVert > 4) throw new IOException("bad V");
            if (tq > 3) throw new IOException("bad TQ");
        ...
        for (int i = 0; i < numComps; i++) {
            ...
            if (c.blocksPerMCUHorz < hMax) c.upsampler |= 1;
            if (c.blocksPerMCUVert < vMax) c.upsampler |= 2;
        }
    }
    ...
    private boolean allocateDecodeTmp(int YUVstride) {
        ...
        for (int compIdx = 0; compIdx < 3; compIdx++) {
            ...
            if (c.upsampler != 0) {
                if (upsampleTmp == null) upsampleTmp = new byte[3][];
    ...
    static public class Huffman {
        ...
        public Huffman(int[] count) throws IOException {
            int numSymbols = 0;
            for (int i = 0; i < 16; i++) numSymbols += count[i];
