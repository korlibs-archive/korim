...
class CanvasContext2dRenderer(private val canvas: HTMLCanvasElementLike) : Context2d.Renderer() {
            ...
            is Context2d.Gradient -> {
                when (kind) {
                    Context2d.Gradient.Kind.LINEAR -> {
                        val grad = ctx.createLinearGradient(this.x0.toDouble(), this.y0.toDouble(), this.x1.toDouble(), this.y1.toDouble())
                        for (n in 0 until this.stops.size) {
                            val stop = this.stops[n]
                            val color = this.colors[n]
                            grad.addColorStop(stop.toDouble(), Colors.toHtmlStringSimple(color))
                        }
                        grad
                    }
                    Context2d.Gradient.Kind.RADIAL -> {
                        val grad = ctx.createRadialGradient(
                            this.x0.toDouble(), this.y0.toDouble(), this.r0.toDouble(),
                            this.x1.toDouble(), this.y1.toDouble(), this.r1.toDouble()
                        )
                        for (n in 0 until this.stops.size) {
                            val stop = this.stops[n]
                            val color = this.colors[n]
    ...
    private fun setState(state: Context2d.State, fill: Boolean) {
        ...
        ctx.setTransform(t.a.toDouble(), t.b.toDouble(), t.c.toDouble(), t.d.toDouble(), t.tx.toDouble(), t.ty.toDouble())
        if (fill) {
    ...
    private fun transformPaint(paint: Context2d.Paint) {
        if (paint is Context2d.TransformedPaint) {
            ...
            ctx.transform(m.a.toDouble(), m.b.toDouble(), m.c.toDouble(), m.d.toDouble(), m.tx.toDouble(), m.ty.toDouble())
        }
    }
    ...
    override fun render(state: Context2d.State, fill: Boolean) {
        ...
        keep {
            ...
            state.path.visitCmds(
                ...
                quadTo = { cx, cy, ax, ay -> ctx.quadraticCurveTo(cx.toDouble(), cy.toDouble(), ax.toDouble(),
                    ay.toDouble()
                ) },
                cubicTo = { cx1, cy1, cx2, cy2, ax, ay -> ctx.bezierCurveTo(cx1.toDouble(),
                    cy1.toDouble(), cx2.toDouble(), cy2.toDouble(), ax.toDouble(), ay.toDouble()
                ) },
                close = { ctx.closePath() }
            )
    ...
    override fun renderText(
        ...
        keep {
            ...
            if (fill) {
                ...
                ctx.strokeText(text, x.toDouble(), y.toDouble())
            }
        }
    }
